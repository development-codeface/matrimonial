...
class Muse
{
	...
	function myshortlist($field_val , $per_page, $page_segment){
		$ci =  & get_instance();
		$ci->load->model('matri');
		$data = $ci->matri->shortlistedprofile($field_val, $per_page, $page_segment);
		return $data;
		
	}

	function myInterested($field_val , $per_page, $page_segment){
		$ci =  & get_instance();
		$ci->load->model('matri');
		$data = $ci->matri->shortlistedInterested($field_val, $per_page, $page_segment);
		return $data;
