...
class Auth extends CI_Controller
{
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->login(
					...
					if (isset($errors['banned'])) {	
						...
						$data['errors']['banned']  = $errors['banned'];

					} elseif (isset($errors['not_activated'])) {				// not activated user
						
					redirect('/auth/send_again/'); //change by shiva
	...
	function register()
	{
		if($this->tank_auth->is_admin_in()){
        ...
        else if ($this->tank_auth->is_logged_in()) {									//change by shiva // logged in
			redirect('/muser/matches');
			...
			$this->form_validation->set_rules('firstname', 'First Name', 'trim|required|xss_clean|min_length['.$this->config->item('firstname_min_length', 'tank_auth').']|max_length['.$this->config->item('firstname_max_length', 'tank_auth').']');
			//$this->form_validation->set_rules('lastname', 'Last Name', 'trim|required|xss_clean|min_length['.$this->config->item('lastname_min_length', 'tank_auth').']|max_length['.$this->config->item('lastname_max_length', 'tank_auth').']');
			...
			$this->form_validation->set_rules('sub_community','Sub Community','trim|strtolower');
			*/
				...
				if (!is_null($data = $this->tank_auth->create_user
					...
					$data['login_by_email'] = $this->config->item('login_by_email', 'tank_auth');
						...
						$this->session->set_userdata(array(
								...
								'status'	=> (TRUE) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
						));
					
					$data['site_name'] = $this->config->item('http://orbitplus.org', 'tank_auth');
