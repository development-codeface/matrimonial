...

{
		...
		$this->form_validation->set_rules('drink','drink','trim|required|strip_tags');
...

            {
                ...
                (
                        ...
                        'religion_id' => (COMMUNITY_CONDITION_ACTIVATED)?SITE_RELIGION_ID:$this->input->post('religion'),



                        'community_id' => (COMMUNITY_CONDITION_ACTIVATED)?SITE_COMMUNITY_ID:$this->input->post('community'),
...

	{
		...
		$this->form_validation->set_rules('whatsapp_no', 'WhatsApp No.', 'trim|required|xss_clean|min_length[9]|max_length[17]');
...

		{
			...
			$this->matri->global_upldate_where('users', array('id'=>$this->tank_auth->get_user_id()),$formdata);
