...

{
...

	{
		...
		$data['matches'] = $this->matri->total_muser_data_new($field_val); //changed by safir
  ...
  //       function community_sites()
		...
		// 	$data['title'] =    'Muser | Home Page | Community Websites';

	 //        // $data['descripation'] ='';

	 //        // $data = $this->matri->model('')

	 //     	$data['websites']= $this->matri->global_select('website_details');

	 //     	$data['religion']= $this->matri->global_select('religion');

	 //     	$data['community']= $this->matri->global_select('community');



		// 	 $data['page'] = 'commen/websites';
...

	{
		...
		if(isset($_POST)){
			...
			// $this->matri->global_upldate_where('user_search',array('user_id'=>$user_id),$insertData);

			$this->matri->insert_update_horo($user_id,$insertData);
...

	{
...

		{
			...
			$this->load->view(SITE_THEME_FOR_VIEW.'muser/filter_page', $data);
	...
	function ajax_getcontact_info(){
		...
		if(isset($packremaining) && $packremaining > 0){
			...
			foreach($userInfo->result() as $row){
				...
				$whatsappNumber = $row->whatsapp_no;
