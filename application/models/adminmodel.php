...
*/
...

    {
        ...
        $this->db->select('users.*, user_profiles.*, education_field.*,
                            ...
                            user_update.description as useraboutus,user_update.abouthobbies as abouthobbies,user_update.id as update_profile_id,nashathram.name as star,user_edu.annual_income as annual_income,user_horoscop.path as horoscope,user_horoscop.birth_time as horoscope_birth_time,user_horoscop.birth_place as horoscope_birth_place');
    ...
    function get_ad_detail($adid) {

        $this->db->select('useradvertisement.*');

        $this->db->from('useradvertisement');
    ...
    function search_users_by_name_or_id($keyword) {

        $this->db->select('users.id, users.firstname, users.email, users.role, user_file.path');

        $this->db->from('users');

        $this->db->join('user_file', 'user_file.user_id = users.id', 'left'); // Left join in case image is missing

        $this->db->like('users.firstname', $keyword);

        $this->db->or_like('users.id', $keyword);
